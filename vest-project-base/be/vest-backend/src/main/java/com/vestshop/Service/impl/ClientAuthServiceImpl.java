...
import com.vestshop.Repository.ClientPasswordResetOtpRepository;
...
public class ClientAuthServiceImpl implements ClientAuthService {
    ...
    private final ClientPasswordResetOtpRepository otpRepo;
    ...
    public ClientAuthServiceImpl(
            ...
            ClientPasswordResetOtpRepository otpRepo,
    ...
    public void sendOtp(String email) {
        ...
        KhachHang kh = khrepo.findByEmail(em).orElse(null);
        if (kh == null) return;

        var lastOtp = otpRepo.findTopByEmailOrderByIdDesc(em);
        if (lastOtp.isPresent() && lastOtp.get().getLastSentAt() != null) {
            long sec = Duration.between(lastOtp.get().getLastSentAt(), LocalDateTime.now()).getSeconds();
        ...
        ClientPasswordResetOtp row = new ClientPasswordResetOtp();
        row.setKhachHangId(kh.getId());
        row.setEmail(em);
        ...
        // dùng template chung cũng được
        emailService.sendResetPasswordOtp(em, kh.getTenKhachHang(), otp);
    }

    @Override
    public void verifyOtp(String email, String otp) {
        getValidOtpRowOrThrowAndIncreaseAttempt(email, otp);
    }
    ...
    public void resetPassword(String email, String otp, String newPassword) {
        ...
        ClientPasswordResetOtp row = otpRepo.findTopByEmailOrderByIdDesc(em)
        ...
        KhachHang kh = khrepo.findByEmail(em)
    ...
    private void getValidOtpRowOrThrowAndIncreaseAttempt(String email, String otp) {
        if (email == null || email.isBlank()) throw new RuntimeException("Email không hợp lệ");
        if (otp == null || otp.isBlank()) throw new RuntimeException("OTP không hợp lệ");
