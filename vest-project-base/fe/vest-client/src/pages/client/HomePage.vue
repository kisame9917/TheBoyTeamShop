<template>
  <div class="homepage">
    <!-- Banner -->
    <section class="hero-banner position-relative" @mouseenter="stopAuto" @mouseleave="startAuto">
      <div
      ...
      >
        <div class="container">
          <div class="col-md-6 text-start hero-content">
            ...
            <button class="btn btn-primary btn-lg rounded-0 px-4 py-2 fw-bold" @click="goShop">
              MUA NGAY
            </button>
          </div>
        </div>
      ...
      <div class="slider-dots text-center position-absolute w-100 bottom-0 mb-3">
        <span
    ...
    <section class="container py-5">
      ...
      <div class="row row-cols-2 row-cols-md-3 row-cols-lg-5 g-4">
        <div class="col" v-for="(product, index) in products" :key="'new-' + product.id">
    ...
    <section class="container py-4">
      ...
      <div class="row row-cols-2 row-cols-md-3 row-cols-lg-5 g-4">
        <div class="col" v-for="(product, index) in exclusiveProducts" :key="'ex-' + product.id">
          <div
              class="card h-100 border-0 product-card cursor-pointer"
              role="button"
              @click="goProduct(product.id)"
    ...
    <!-- Banner giữa -->
    <section class="container py-4">
      <div class="mid-banner bg-brilliant-azure text-white d-flex align-items-center justify-content-between rounded overflow-hidden">
        <div class="p-5 ms-4">
          <h2 class="fw-bold">BỘ SƯU TẬP VEST CÔNG SỞ</h2>
          <p class="fs-5">Bộ sưu tập vest</p>
          <button class="btn btn-light text-primary fw-bold px-4 rounded-0 mt-3" @click="goShop">
            MUA NGAY
          </button>
        </div>
...
.bg-brilliant-azure { background-color: var(--brilliant-azure); }
.hero-banner { height: 500px; background-color: var(--pale-blue-bg); overflow: hidden; }
.hero-bg { height: 100%; background-position: center right; background-repeat: no-repeat; background-size: cover; transition: background-image 0.35s ease; }
.hero-content { background: rgba(232, 246, 250, 0.8); padding: 2rem; border-radius: 8px; }
.slider-btn { position: absolute; top: 50%; transform: translateY(-50%); background: transparent; border: none; font-size: 2rem; color: var(--royal-blue); cursor: pointer; }
.prev-btn { left: 20px; }
.next-btn { right: 20px; }
.dot { display: inline-block; width: 30px; height: 4px; background-color: #ccc; margin: 0 4px; cursor: pointer; }
.dot.active { background-color: var(--royal-blue); }
.card-img-wrapper { aspect-ratio: 3/4; overflow: hidden; border-radius: 4px; }
...
.gallery-card { border-radius: 12px; overflow: hidden; background: #f1f3f5; height: 380px; }
