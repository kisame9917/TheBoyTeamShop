<template>
  <div class="container-fluid py-3">
        ...
        <div class="d-flex align-items-center justify-content-between gap-2 flex-wrap">
          <div class="d-flex flex-wrap gap-2">
            ...
            >
              ...
              <span class="tab-x" title="Đóng" @click.stop="closeOrder(o.id)">×</span>
            </button>
          </div>
        ...
        <div v-else>
          <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-2">
            ...
            <div class="d-flex gap-2">
              <button class="btn btn-outline-primary btn-sm" type="button" @click="openProductModal">
                <i class="bi bi-plus-lg me-1"></i> Thêm sản phẩm
              </button>
              <button class="btn btn-outline-primary btn-sm" type="button" @click="toastInfo('Chưa tích hợp quét QR')">
                ...
                <tbody>
                  <tr v-for="(it, idx) in activeOrder.cart" :key="it.key">
                    ...
                    <td>
                      ...
                      <div v-if="it.priceChangedLocked" class="small text-danger fw-semibold mt-1">
                      ...
                      <div v-if="it.lineStatus === 'missing'" class="small text-danger fw-semibold mt-1">
                        Biến thể này không còn tồn tại hoặc chưa tải được dữ liệu mới.
                      </div>
                    </td>
                    ...
                    <td class="text-center">
                      <div class="btn-group" role="group">
                        <button
                          ...
                          @click="decQty(idx)"
                          :disabled="it.priceChangedLocked || it.lineStatus === 'missing'"
                        >
                          -
                        </button>
            ...
            <div class="col-12 col-lg-6">
              <div class="card h-100">
                <div class="card-header bg-white d-flex align-items-center justify-content-between flex-wrap gap-2">
                  <div>
                    <div class="fw-bold">{{ activeOrder.loaiDon ? 'Thông tin giao hàng' : 'Thông tin khách hàng' }}</div>
                    <div class="text-muted small">
                      Mã HĐ:
                      <span class="fw-semibold font-monospace">{{ activeOrder.maHoaDon }}</span>
                    </div>
                  </div>

                  <div class="d-flex align-items-center gap-2">
                    <button class="btn btn-outline-secondary btn-sm customer-action-btn" type="button" @click="resetToWalkInCustomer">
                      Đặt lại
                    </button>

                    <button class="btn btn-outline-dark btn-sm customer-action-btn" type="button" @click="openCustomerModal">
                      Chọn khách hàng
                    </button>
                  </div>
                </div>
                ...
                <div class="card-body">
                  ...
                  <template v-else>
                    <div class="border rounded-3 p-3">
                      ...
                      <div class="row g-2">
                        <div class="col-12 col-md-6">
                          <input class="form-control" placeholder="Tên người nhận" v-model.trim="activeOrder.tenNguoiNhanHang" />
                        </div>
                        <div class="col-12 col-md-6">
                          <input class="form-control" placeholder="SĐT người nhận" v-model.trim="activeOrder.soDienThoaiNhanHang" />
                        </div>

                        <div class="col-12 col-md-6">
                          <label class="form-label mb-1">Tỉnh/Thành</label>
                          <select class="form-select" v-model="activeOrder.tinhThanhNhanHang" @change="onProvinceChange($event.target.value)" :disabled="provincesLoading">
                        ...
                        <div class="col-12 col-md-6">
                          ...
                          <select class="form-select" v-model="activeOrder.phuongXaNhanHang" :disabled="!activeOrder.tinhThanhNhanHang || wardsLoading">
                        ...
                        <div class="col-12">
                          ...
                          <input class="form-control" placeholder="Số nhà, tên đường..." v-model.trim="activeOrder.diaChiNhanHangChiTiet" />
                        </div>

                        <div class="col-12 mt-3">
                          <div class="shipping-brand-box">
                            <img :src="ghnLogo" alt="GHN Express" class="shipping-brand-logo" />
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div class="card-body">
                  <div class="mb-3">
                    ...
                    <div class="form-check form-switch">
                      <input class="form-check-input" type="checkbox" :checked="!!activeOrder.loaiDon" @change="toggleShip" id="switch-ship" />
                      <label class="form-check-label" for="switch-ship">
                        {{ activeOrder.loaiDon ? 'Bán tại quầy (có ship)' : 'Bán tại quầy' }}
                      </label>
                    </div>
                  </div>
                  ...
                  <div class="mb-3">
                    <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                      ...
                      <div class="d-flex gap-2">
                        <button class="btn btn-outline-secondary btn-sm" type="button" @click="reloadActiveOrderVouchers">
                          Tải lại
                        </button>
                    ...
                    <div class="border border-top-0 rounded-bottom p-2">
                      <template v-if="activeOrder.voucherTab === 'best'">
                        ...
                        <div v-else-if="bestVoucherEntryUI" class="voucher-card">
                          <div class="d-flex align-items-start justify-content-between gap-2">
                            <div>
                              <div class="d-flex gap-2 flex-wrap">
                                <span class="badge text-bg-primary">PGG</span>
                                <span class="badge text-bg-secondary">Mã tốt nhất</span>
                                <span class="badge text-bg-dark">{{ bestVoucherEntryUI.v.ma_giam_gia }}</span>

                                <span v-if="activeOrder.pggId === bestVoucherEntryUI.v.id && bestVoucherEntryUI.discount <= 0" class="badge text-bg-danger">
                                  Không còn hiệu lực
                                </span>
                              </div>

                              <div class="mt-2 fw-bold">{{ bestVoucherEntryUI.v.ten_giam_gia }}</div>

                              <div class="mt-2">
                                <span class="text-muted">Giảm:</span>
                                <span class="ms-2 fw-bold text-danger">- {{ money(bestVoucherEntryUI.discount) }}</span>
                              </div>

                              <div class="small text-muted mt-1">
                                <span v-if="bestVoucherEntryUI.v.don_hang_toi_thieu > 0">
                                  Đơn tối thiểu:
                                  <b>{{ money(bestVoucherEntryUI.v.don_hang_toi_thieu) }}</b>
                                </span>
                                <span v-if="bestVoucherEntryUI.v.gia_tri_giam_toi_da > 0" class="ms-2">
                                  Tối đa:
                                  <b>{{ money(bestVoucherEntryUI.v.gia_tri_giam_toi_da) }}</b>
                                </span>
                              </div>
                            </div>

                            <div class="text-end">
                              <span v-if="activeOrder.voucherMode !== 'none' && activeOrder.pggId === bestVoucherEntryUI.v.id" class="badge text-bg-success">
                                Đang áp dụng
                              </span>

                              <button v-else class="btn btn-success btn-sm" type="button" @click="applyVoucherManual(bestVoucherEntryUI.v)">
                                Áp dụng
                              </button>
                            </div>
                          </div>
                          ...
                          <div class="mt-3 small text-muted">
                            <div>
                              ...
                              <b>{{ formatDateVN(bestVoucherEntryUI.v.ngay_ket_thuc) }}</b>
                            </div>
                          </div>
                        </div>

                        <div v-else class="text-muted small py-2">Không có mã giảm giá phù hợp với đơn hiện tại.</div>
                      </template>

                      <template v-else>
                        <div v-if="altVoucherEntriesUI.length === 0" class="text-muted small py-2">
                          Không có mã thay thế khả dụng.
                        </div>

                        <div v-else class="d-flex flex-column gap-2">
                          <div v-for="e in altVoucherEntriesUI.slice(0, 10)" :key="e.v.id" class="voucher-card">
                            <div class="d-flex align-items-start justify-content-between gap-2">
                              <div>
                                <div class="d-flex gap-2 flex-wrap">
                                  <span class="badge text-bg-primary">PGG</span>
                                  ...
                                  <span v-if="activeOrder.pggId === e.v.id && activeOrder.voucherMode !== 'none'" class="badge text-bg-success">
                                    Đang chọn
                                  </span>

                                  <span v-if="activeOrder.pggId === e.v.id && e.discount <= 0" class="badge text-bg-danger">
                                    Không còn hiệu lực
                                  </span>
                                </div>
                                ...
                                <div class="small text-muted mt-1">
                                  <span v-if="e.v.don_hang_toi_thieu > 0">Đơn tối thiểu: <b>{{ money(e.v.don_hang_toi_thieu) }}</b></span>
                                  <span v-if="e.v.gia_tri_giam_toi_da > 0" class="ms-2">Tối đa: <b>{{ money(e.v.gia_tri_giam_toi_da) }}</b></span>
                                </div>
                              </div>
                    ...
                    <div class="mt-3">
                      ...
                      <div v-if="voucherSuggestions.length === 0" class="text-muted small mt-1">
                        Không có gợi ý mua thêm để đạt mã tốt hơn
                      </div>

                      <div v-else class="d-flex flex-column gap-2 mt-2">
                        <div v-for="e in voucherSuggestions" :key="'suggest-' + e.v.id" class="border rounded-3 p-2">
                          <div class="d-flex align-items-center justify-content-between gap-2 flex-wrap">
                            <div>
                              Thêm <b class="text-danger">{{ money(e.missing) }}</b>
                              để dùng <b>{{ e.v.ma_giam_gia }}</b> (giảm khoảng
                              <b class="text-danger">-{{ money(e.expectedDiscount) }}</b>
                              khi đơn ≥ {{ money(e.minOrder) }})
                            </div>

                            <button class="btn btn-outline-secondary btn-sm" type="button" @click="toastInfo('Thêm sản phẩm để đủ điều kiện rồi áp dụng mã')">
                              Xem
                            </button>
                          </div>
                        </div>
                  ...
                  <ul class="list-group mb-3">
                    ...
                    <li v-if="activeOrder.loaiDon" class="list-group-item d-flex justify-content-between">
                  ...
                  <div class="d-grid gap-2">
                    <button class="btn btn-success w-100" :disabled="activeOrder.cart.length === 0 || submitting" @click="confirmOrderCash">
                      Thanh toán (tiền mặt)
                    </button>

                    <button class="btn btn-outline-success w-100" type="button" :disabled="activeOrder.cart.length === 0 || submitting" @click="openQrPay">
                      Thanh toán QR
                    </button>
                  </div>
    ...
    <div v-if="toast.show" class="position-fixed top-0 end-0 p-3" style="z-index: 2000">
      <div class="custom-toast shadow-sm" :class="toastClass">
        ...
        <button type="button" class="custom-toast-close" @click="toast.show = false" aria-label="Close">×</button>
      </div>
    </div>

    <teleport to="body">
      <div v-if="backdropOpen" class="modal-backdrop fade show" @click="closeBackdropModal" style="z-index: 1050"></div>

      <div v-if="showProductModal" class="modal fade show" tabindex="-1" role="dialog" aria-modal="true" style="display: block; z-index: 1055">
            ...
            <div class="modal-body">
                ...
                <div class="card-body pt-3">
                  <div class="row g-3 align-items-end">
                    <div class="col-12 col-lg-6">
                      ...
                      <input class="form-control" placeholder="Tìm theo mã / tên sản phẩm..." v-model.trim="productFilters.keyword" />
                    </div>
                    ...
                    <div class="col-12 col-md-6 col-lg-3">
                      ...
                      <select class="form-select" v-model="productFilters.color">
                        ...
                        <option v-for="c in productColorOptions" :key="c" :value="c">{{ c }}</option>
                      </select>
                    </div>
                    ...
                    <div class="col-12 col-lg-7">
                      ...
                      <div class="d-flex align-items-center gap-2 flex-wrap mb-2">
                        <span class="fw-semibold text-success">{{ money(productPriceRange.min).replace(' đ', '') }}</span>
                        <span>-</span>
                        <span class="fw-semibold text-success">{{ money(productPriceRange.max).replace(' đ', '') }}</span>
                      </div>

                      <div class="price-range-wrap">
                        <input class="range-input" type="range" :min="productPriceBounds.min" :max="productPriceBounds.max" :step="productPriceStep" v-model.number="productPriceRange.min" @input="onPriceRangeMinInput" />

                        <input class="range-input" type="range" :min="productPriceBounds.min" :max="productPriceBounds.max" :step="productPriceStep" v-model.number="productPriceRange.max" @input="onPriceRangeMaxInput" />
                      </div>
                    ...
                    <div class="col-12 col-lg-5">
                      ...
                      <div class="product-status-group">
                        <div class="form-check">
                          <input class="form-check-input" type="radio" value="" v-model="productFilters.stockStatus" id="stock-all" />
                          <label class="form-check-label" for="stock-all">Tất cả</label>
                        </div>

                        <div class="form-check">
                          <input class="form-check-input" type="radio" value="in" v-model="productFilters.stockStatus" id="stock-in" />
                          <label class="form-check-label" for="stock-in">Còn hàng</label>
                        </div>

                        <div class="form-check">
                          <input class="form-check-input" type="radio" value="out" v-model="productFilters.stockStatus" id="stock-out" />
                    ...
                    <div class="col-12 d-flex justify-content-between align-items-center flex-wrap gap-2">
                      ...
                      <div class="d-flex gap-2">
                        <button class="btn btn-outline-secondary" type="button" @click="resetProductFilters">Đặt lại</button>

                        <button class="btn btn-outline-secondary" type="button" @click="reloadProducts" :disabled="productLoading">Tải lại</button>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <table class="table table-bordered align-middle">
                  ...
                  <tbody>
                    <tr v-for="(p, i) in filteredProducts" :key="p.idSpct">
                      ...
                      <td class="text-center">
                        <button class="btn btn-dark btn-sm" :disabled="p.stock <= 0" @click="chooseProduct(p)">
                          Chọn
                        </button>
                      </td>
                    </tr>
              ...
              <div class="d-flex align-items-center mt-2">
                <div class="text-muted small" style="min-width: 220px">
                  Hiển thị {{ filteredProducts.length }} / tổng {{ productTotal }} bản ghi
                </div>

                <div class="flex-grow-1 d-flex justify-content-center align-items-center gap-2">
                  <button class="btn btn-outline-dark btn-sm" @click="productPrev" :disabled="productPage <= 0 || productLoading">‹</button>

                  <div class="small">
                    Trang <b>{{ productPage + 1 }}</b>
                    <span v-if="productTotalPages">/ {{ productTotalPages }}</span>
                  </div>

                  <button class="btn btn-outline-dark btn-sm" @click="productNext" :disabled="productLast || productLoading">›</button>
                </div>

                <div class="d-flex justify-content-end" style="min-width: 190px">
                  <select class="form-select form-select-sm" style="width: 150px" v-model.number="productSize" @change="onProductSizeChange">
      ...
      <div v-if="showCustomerModal" class="modal fade show" tabindex="-1" role="dialog" aria-modal="true" style="display: block; z-index: 1055">
        <div class="modal-dialog modal-xl modal-dialog-scrollable customer-modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
              <div class="d-flex gap-2 mb-3 flex-wrap">
                ...
                <div class="ms-auto small text-muted d-flex align-items-center gap-2">
                  ...
                  <span>Tổng: <b>{{ customerTotal }}</b></span>
                </div>
              </div>

              <div class="table-responsive customer-table-wrap">
                <table class="table table-bordered align-middle mb-0 customer-table">
                  <colgroup>
                    <col style="width: 8%" />
                    <col style="width: 25%" />
                    <col style="width: 18%" />
                  ...
                  <tbody>
                    <tr v-for="(c, i) in pagedCustomers" :key="c.id || c.phone || i">
                      <td class="text-center">{{ i + 1 + customerPage * customerSize }}</td>
                      <td class="fw-semibold">{{ c.name }}</td>
                      <td>{{ c.phone }}</td>
                      <td class="customer-address-cell">{{ c.address }}</td>
                      <td class="text-center">
                        <button class="btn btn-dark btn-sm customer-pick-btn" @click="chooseCustomer(c)">
                          Chọn
                        </button>
                      </td>
                    </tr>
              ...
              <div class="d-flex align-items-center mt-2">
                <div class="text-muted small" style="min-width: 220px">
                  Hiển thị {{ pagedCustomers.length }} / tổng {{ customerTotal }} bản ghi
                </div>

                <div class="flex-grow-1 d-flex justify-content-center align-items-center gap-2">
                  <button class="btn btn-outline-dark btn-sm" @click="customerPrev" :disabled="customerPage <= 0 || customerLoading">‹</button>

                  <div class="small">
                    Trang <b>{{ customerPage + 1 }}</b>
                    <span v-if="customerTotalPages">/ {{ customerTotalPages }}</span>
                  </div>

                  <button class="btn btn-outline-dark btn-sm" @click="customerNext" :disabled="customerLast || customerLoading">›</button>
                </div>

                <div class="d-flex justify-content-end" style="min-width: 190px">
                  <select class="form-select form-select-sm" style="width: 150px" v-model.number="customerSize" @change="onCustomerSizeChange">
                    <option :value="10">10 bản ghi / trang</option>
                    <option :value="20">20 bản ghi / trang</option>
                    <option :value="50">50 bản ghi / trang</option>
                  </select>
      ...
      <div v-if="showQrPayModal" class="modal fade show" tabindex="-1" role="dialog" aria-modal="true" style="display: block; z-index: 1065">
        <div class="modal-dialog modal-md modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-footer">
              <button class="btn btn-outline-secondary" type="button" @click="closeQrPay">
                Đóng
              </button>
      ...
      <div v-if="showPaymentConfirmModal" class="modal fade show" tabindex="-1" role="dialog" aria-modal="true" style="display: block; z-index: 1068">
        <div class="modal-dialog modal-md modal-dialog-centered">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title fw-bold">Xác nhận thanh toán</h5>
              <button type="button" class="btn-close" @click="resolvePaymentConfirm(false)"></button>
            </div>
            ...
            <div class="modal-footer">
              <button class="btn btn-outline-secondary" type="button" @click="resolvePaymentConfirm(false)">Hủy</button>
              <button class="btn btn-success" type="button" @click="resolvePaymentConfirm(true)">Xác nhận</button>
            </div>
          </div>
        </div>
      </div>

      <div v-if="showPreCheckoutModal" class="modal fade show" tabindex="-1" role="dialog" aria-modal="true" style="display: block; z-index: 1060">
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div v-if="preCheckoutUi.suggest" class="mt-2 small">
                ...
                • giảm
                <b class="text-danger">-{{ money(preCheckoutUi.suggest.discount) }}</b>
              </div>
            </div>

            <div class="modal-footer">
              <button class="btn btn-outline-secondary" type="button" @click="resolvePreCheckout(false)">Không</button>
              <button class="btn btn-success" type="button" @click="resolvePreCheckout(true)">Có</button>
            </div>
          </div>
        </div>
      </div>
...
const MAX_ORDERS = 10;
const STORAGE_KEY = 'sales_store_only_v2_fixed';
...
const toast = reactive({ show: false, title: '', msg: '', type: 'danger' });
...
function toastShow(msg, type = 'danger', title = '') {
  ...
  toast.msg = msg;
  toast.type = type;
  toast.title = title || (type === 'success' ? 'Thành công' : type === 'warning' ? 'Cảnh báo' : type === 'info' ? 'Thông báo' : 'Thất bại');
...
async function createOrder() {
  ...
  try {
    ...
    const data = res?.data || {};
    orders.value.push(normalizeOrder({ id: localId, dbId: data.id, maHoaDon: data.maHoaDon || maHoaDon, label: `Hóa Đơn - ${data.maHoaDon || maHoaDon}` }));
...
async function closeOrder(id) {
  ...
  if (o?.dbId) {
    try {
      const items = (o.cart || []).map((it) => ({ idSanPhamChiTiet: Number(it.idSpct), soLuong: Number(it.qty || 0) })).filter((x) => x.idSanPhamChiTiet && x.soLuong > 0);
      await http.post(`/api/hoa-don/draft/${o.dbId}/cancel`, { reason: 'Đóng tab bán hàng', items });
    } catch (e) {
      console.error(e);
    }
  }
...
function saveDraftsNow() {
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ dateKey: getDateKeyLocal(), orders: orders.value, activeId: activeId.value, orderSeq: orderSeq.value, savedAt: new Date().toISOString() }));
  } catch {}
}
...
const anyModalOpen = computed(() => showProductModal.value || showCustomerModal.value || showQrPayModal.value || showPreCheckoutModal.value || showPaymentConfirmModal.value);
const backdropOpen = computed(() => showProductModal.value || showCustomerModal.value || showQrPayModal.value);
watch(anyModalOpen, (open) => document.body.classList.toggle('modal-open', open));
...
const productFilters = reactive({ keyword: '', color: '', size: '', stockStatus: '' });
const productPriceRange = reactive({ min: 0, max: 100000000 });
...
function mapSpct(x) {
  ...
  return {
    ...
    image: buildImgUrl(x.anh || x.anhDaiDien || x.primaryImageUrl || x.mediaAsset),
  };
}
function getVariantFingerprint(obj) {
  return [String(obj?.name || '').trim().toLowerCase(), String(obj?.color || '').trim().toLowerCase(), String(obj?.size || '').trim().toLowerCase()].join('|');
}
...
function findRepricedProductInList(cartItem, list) {
  const sameId = list.find((p) => Number(p.idSpct) === Number(cartItem.idSpct) && !sameMoney(p.price, cartItem.price));
  if (sameId) return sameId;
  const fp = getVariantFingerprint(cartItem);
  return list.find((p) => getVariantFingerprint(p) === fp && !sameMoney(p.price, cartItem.price)) || null;
}
...
async function fetchAllProductsForRefresh() {
  const firstRes = await getAllDetails(0, 100);
  const firstData = firstRes?.data ?? firstRes;
  if (Array.isArray(firstData)) return firstData.map(mapSpct);
...
async function reloadProducts() { await fetchProducts(0); }
function productPrev() { if (productPage.value <= 0) return; fetchProducts(productPage.value - 1); }
function productNext() { if (productLast.value) return; fetchProducts(productPage.value + 1); }
function onProductSizeChange() { fetchProducts(0); }
...
watch(productPriceBounds, (bounds) => {
  if (productPriceRange.min < bounds.min || productPriceRange.min > bounds.max) productPriceRange.min = bounds.min;
  if (productPriceRange.max > bounds.max || productPriceRange.max < bounds.min) productPriceRange.max = bounds.max;
  if (productPriceRange.min > productPriceRange.max) {
    productPriceRange.min = bounds.min;
    productPriceRange.max = bounds.max;
  }
}, { immediate: true });
function onPriceRangeMinInput() { if (productPriceRange.min > productPriceRange.max) productPriceRange.min = productPriceRange.max; }
function onPriceRangeMaxInput() { if (productPriceRange.max < productPriceRange.min) productPriceRange.max = productPriceRange.min; }

const productColorOptions = computed(() => [...new Set(products.value.map((p) => p.color).filter(Boolean))].sort((a, b) => String(a).localeCompare(String(b), 'vi')));
const productSizeOptions = computed(() => [...new Set(products.value.map((p) => p.size).filter(Boolean))].sort((a, b) => String(a).localeCompare(String(b), 'vi')));
const filteredProducts = computed(() => {
  ...
  return products.value.filter((p) => {
    const textMatched = !kw || [p.code, p.name, p.color, p.size].some((x) => String(x || '').toLowerCase().includes(kw));
    const colorMatched = !color || String(p.color || '').toLowerCase() === color;
    const sizeMatched = !size || String(p.size || '').toLowerCase() === size;
    const stockMatched = !stockStatus || (stockStatus === 'in' && Number(p.stock || 0) > 0) || (stockStatus === 'out' && Number(p.stock || 0) <= 0);
    const price = Number(p.price || 0);
    const priceMatched = price >= minPrice && price <= maxPrice;
    return textMatched && colorMatched && sizeMatched && stockMatched && priceMatched;
  });
});
...
async function setQtyByInput(cartIndex, nextQtyRaw) {
  ...
  if (it.priceChangedLocked || it.lineStatus === 'missing') {
    return toastShow(`Sản phẩm ${it.code} đã đổi giá hoặc không còn hợp lệ, vui lòng xóa dòng cũ và chọn lại`, 'warning');
  }
  ...
  try {
    ...
    const fallbackMsg = remain <= 0 ? `Sản phẩm ${it.code} hiện đã hết tồn kho, còn lại tồn ${remain}` : `Số lượng mua không được vượt quá số lượng tồn kho còn lại tồn ${remain}`;
    const msg = String(rawMsg).trim() === 'Không đủ tồn kho' ? fallbackMsg : rawMsg || fallbackMsg;
    return toastShow(String(msg), 'warning');
  }
  const p = findModalProductById(it.idSpct);
  if (p) p.stock = Math.max(0, Number(p.stock || 0) - delta);
  it.qty = nextQty;
  syncAllCartStocks(o);
}
...
async function removeItem(i) {
  ...
  if (!it) return;
  try {
    if (it.lineStatus !== 'missing') await increaseStock(Number(it.idSpct), Number(it.qty || 0));
  } catch (err) {
...
async function chooseProduct(p) {
  ...
  if ((Number(p.stock) || 0) <= 0) return toastShow(`Sản phẩm ${p.code} hiện đã hết tồn kho`, 'warning');
...
async function reloadCustomers() { await fetchCustomersAll(); }
function customerPrev() { if (customerPage.value <= 0) return; customerPage.value--; }
function customerNext() { if (customerLast.value) return; customerPage.value++; }
function onCustomerSizeChange() { customerPage.value = 0; }
...
const pagedCustomers = computed(() => {
  ...
  return filteredCustomers.value.slice(start, start + customerSize.value);
});
const customerTotalPages = computed(() => Math.max(1, Math.ceil(filteredCustomers.value.length / customerSize.value)));
const customerLast = computed(() => customerPage.value >= customerTotalPages.value - 1);
const customerTotal = computed(() => filteredCustomers.value.length);
watch(customerKw, () => (customerPage.value = 0));
...
async function chooseCustomer(c) {
  ...
  closeCustomerModal();
  await reloadActiveOrderVouchers();
}
async function resetToWalkInCustomer() {
  const o = activeOrder.value;
  if (!o) return;
  o.customer = null;
...
function onProvinceChange(provinceCode) {
  ...
  try {
    ...
    wards.value = (p?.wardsRaw || []).map(normWard).filter(Boolean).sort((a, b) => a.name.localeCompare(b.name, 'vi'));
  } finally {
...
function pickId(obj) {
  ...
  return obj.id ?? obj.khachHangId ?? obj.idKhachHang ?? obj.value ?? obj.khachHang?.id ?? obj.customer?.id ?? obj.khach_hang?.id ?? null;
}
...
async function loadVouchersByCustomer(customerId) {
  try {
    const res = await http.get('/api/pgg/pos', { params: { khachHangId: customerId ?? null } });
...
async function reloadActiveOrderVouchers() {
  activeOrderVouchers.value = await loadVouchersByCustomer(activeOrder.value?.customer?.id ?? null);
}
...
function voucherBelongsToCustomer(v, customerId) {
  ...
  if (Array.isArray(v.khach_hang_ids) && v.khach_hang_ids.length) return v.khach_hang_ids.includes(Number(customerId));
  if (v.khach_hang_id != null) return Number(v.khach_hang_id) === Number(customerId);
  return true;
}
...
function clearVoucherSuggestionDismissed(o) { if (o) o.voucherSuggestionDismissedKey = ''; }
function toTime(d) {
  if (!d) return null;
...
function getVoucherEntriesForOrder(order, vouchers) {
  const st = order ? order.cart.reduce((s, it) => s + (Number(it.price) || 0) * (Number(it.qty) || 0), 0) : 0;
  return vouchers.map((v) => ({ v, discount: calcVoucherDiscount(st, v) })).sort((a, b) => b.discount - a.discount);
}

const eligibleVoucherEntries = computed(() => getVoucherEntriesForOrder(activeOrder.value, activeOrderVouchers.value).filter((x) => x.discount > 0));
const bestEligibleVoucherEntry = computed(() => eligibleVoucherEntries.value[0] || null);
const altEligibleVoucherEntries = computed(() => [...eligibleVoucherEntries.value].sort((a, b) => a.discount - b.discount));
...
const altVoucherEntriesUI = computed(() => {
  const o = activeOrder.value;
  const applied = appliedVoucherEntry.value;
  if (o?.pggId && applied && applied.discount <= 0) return [...eligibleVoucherEntries.value].sort((a, b) => a.discount - b.discount);
  return altEligibleVoucherEntries.value;
});
watch([subTotal, activeOrderVouchers, activeId, () => activeOrder.value?.customer?.id], () => {
  const o = activeOrder.value;
  if (!o) return;
...
watch([activeId, () => activeOrder.value?.customer?.id], () => { reloadActiveOrderVouchers(); }, { immediate: true });
...
const voucherSuggestions = computed(() => {
  ...
  return activeOrderVouchers.value
    .filter((v) => v?.trang_thai && (Number(v.so_luong) || 0) > 0 && isVoucherInDateRange(v))
    .map((v) => {
    ...
    .filter(Boolean)
    .sort((a, b) => a.missing - b.missing || b.expectedDiscount - a.expectedDiscount)
    .slice(0, 6);
});
...
function applyVoucherManual(v) {
  ...
  if (isPersonalVoucher(v)) {
    ...
    if (!voucherBelongsToCustomer(v, cid)) return toastShow('Voucher này không thuộc khách hàng hiện tại', 'warning');
  }
...
function applyBestVoucherNowForOrder(o, vouchers) {
  if (!o) return;
  const best = getVoucherEntriesForOrder(o, vouchers).filter((x) => x.discount > 0)[0];
  if (!best?.v) return;
  o.voucherMode = 'manual';
  o.pggId = best.v.id;
...
const grandTotal = computed(() => Math.max(0, subTotal.value - discountMoney.value + shipMoney.value));
...
const preCheckoutUi = reactive({ type: 'info', message: '', detail: '', suggest: null });
...
function openPaymentConfirm(text) {
  paymentConfirmText.value = text || 'Bạn có chắc muốn thanh toán hóa đơn này không?';
  showPaymentConfirmModal.value = true;
  return new Promise((resolve) => { _resolvePaymentConfirm = resolve; });
}
...
function getVoucherChangedFields(snap, vNow) {
  ...
  if (endOld !== endNew) changes.push(`Hạn dùng: ${formatDateVN(endOld)} → ${formatDateVN(endNew)}`);
...
async function runVoucherPrecheckFlow() {
  ...
  const vNow = orderVouchers.find((x) => x.id === o.pggId) || null;
  const bestEntry = getVoucherEntriesForOrder(o, orderVouchers).filter((x) => x.discount > 0)[0] || null;
  const best = bestEntry ? { id: bestEntry.v.id, code: bestEntry.v.ma_giam_gia, discount: bestEntry.discount } : null;
  if (!vNow) {
    const ok = await openConfirm({
      type: 'danger',
      message: `Voucher "${o.voucherCode || snap?.code || ''}" hiện không còn khả dụng.`,
      detail: best ? `Bạn có muốn đổi sang mã tốt nhất hiện tại (${best.code}) không?` : 'Không có mã khác áp dụng được. Bạn có muốn bỏ voucher để tiếp tục thanh toán không?',
      suggest: best,
    });
    if (!ok) return false;
    if (best) applyBestVoucherNowForOrder(o, orderVouchers);
  ...
  if (snap) {
    ...
    if (changes.length > 0) {
      const ok = await openConfirm({ type: 'danger', message: `Điều kiện voucher "${snap.code || o.voucherCode}" đã thay đổi.`, detail: changes.join(' • '), suggest: best });
      if (!ok) return false;
      if (best && best.id !== vNow.id) applyBestVoucherNowForOrder(o, orderVouchers);
  ...
  if (best && best.id !== vNow.id && best.discount > currentDiscount) {
    ...
    if (ok) applyBestVoucherNowForOrder(o, orderVouchers);
  }
  return true;
}
...
function closeQrPay() { showQrPayModal.value = false; }
async function markPaidAndCheckout() {
  const o = activeOrder.value;
  if (!o) return;
  const ok = await openPaymentConfirm(`Bạn xác nhận đã nhận tiền QR cho hóa đơn ${o.maHoaDon} không?`);
  if (!ok) return;
  o.paid = grandTotal.value;
  o.maGiaoDich = `QR-${o.maHoaDon}-${Date.now()}`;
  o.ghiChuThanhToan = (qrNoteDraft.value || `QR thanh toán - ${o.maHoaDon}`).trim();
  showQrPayModal.value = false;
  await confirmOrder();
}
...
function validateCheckout(o) {
  ...
  for (const it of o.cart) {
    if (it.priceChangedLocked) return `Sản phẩm ${it.code} đã thay đổi giá, vui lòng xóa dòng cũ và chọn lại dòng giá mới`;
    if (it.lineStatus === 'missing') return `Sản phẩm ${it.code} không còn hợp lệ, vui lòng xóa khỏi giỏ`;
  ...
  if (needShip) {
    ...
    if (!String(o.tinhThanhNhanHang || '').trim()) return 'Vui lòng chọn Tỉnh/Thành';
    if (!String(o.phuongXaNhanHang || '').trim()) return 'Vui lòng chọn Phường/Xã';
...
function buildPosPayload(o) {
  ...
  return {
    ...
    ghiChuThanhToan: o.paymentMethod === 'QR' ? o.ghiChuThanhToan || null : null,
    ...
    diaChiNhanHangChiTiet: isShip ? (o.diaChiNhanHangChiTiet || '').trim() : null,
    items: o.cart.map((it) => ({ idSanPhamChiTiet: Number(it.idSpct), soLuong: clampInt(it.qty, 1, 999999) })),
  };
}
async function resetOrderAfterPaid(o) {
  const idx = orders.value.findIndex((x) => x.id === o.id);
  if (idx !== -1) orders.value.splice(idx, 1);
  if (activeId.value === o.id) activeId.value = orders.value[0]?.id ?? null;
  saveDraftsNow();
}
async function confirmOrderCash() {
  const o = activeOrder.value;
  if (!o) return;
  const ok = await openPaymentConfirm(`Bạn có chắc muốn thanh toán hóa đơn ${o.maHoaDon} bằng tiền mặt không?`);
  if (!ok) return;
...
function handleDocumentVisibility() {
  ...
  if (document.visibilityState === 'visible') queueRefreshWhenVisible();
}
async function refreshProductsInCartAndModal() {
  let latestAll = [];
  try {
    latestAll = await fetchAllProductsForRefresh();
  } catch (e) {
    console.error(e);
    toastShow('Không đồng bộ được dữ liệu sản phẩm mới nhất', 'warning');
    return;
  }
  if (showProductModal.value) await fetchProducts(productPage.value || 0);
  for (const order of orders.value) {
    if (!order || !Array.isArray(order.cart)) continue;
    for (const item of order.cart) {
      const latest = latestAll.find((p) => Number(p.idSpct) === Number(item.idSpct)) || null;
      const repriced = findRepricedProductInList(item, latestAll);
      if (!latest && !repriced) {
        item.stockBase = 0;
        item.stock = 0;
...
async function processVoucherChangesForAllOrders() {
  for (const order of orders.value) {
    ...
    const orderVouchers = await loadVouchersByCustomer(order?.customer?.id ?? null);
    const currentVoucher = order.pggId ? orderVouchers.find((v) => v.id === order.pggId) || null : null;
    const subtotal = order.cart.reduce((s, it) => s + (Number(it.price) || 0) * (Number(it.qty) || 0), 0);
    const bestEntry = getVoucherEntriesForOrder(order, orderVouchers).filter((x) => x.discount > 0)[0] || null;
    if (!currentVoucher || getVoucherInvalidReason(currentVoucher, subtotal)) {
      if (bestEntry?.v) applyBestVoucherNowForOrder(order, orderVouchers);
      else removeVoucherNowForOrder(order);
    }
    if (order.id === activeId.value) activeOrderVouchers.value = orderVouchers;
  }
}
...
async function handleTabSync(event) {
  ...
  if (msg.type === TAB_SYNC_EVENTS.PRODUCT_CHANGED || msg.type === TAB_SYNC_EVENTS.PRODUCT_STOCK_CHANGED) {
    await refreshProductsInCartAndModal();
  }
}
...
.voucher-card { border: 1px solid #bfead8; background: #e9fbf3; border-radius: 10px; padding: 12px; }
...
.product-status-group { display: flex; align-items: center; gap: 24px; flex-wrap: wrap; min-height: 38px; }
.product-status-group .form-check { display: flex; align-items: center; gap: 6px; margin-bottom: 0; }
.price-range-wrap { position: relative; height: 24px; display: flex; align-items: center; }
.range-input { width: 100%; appearance: none; background: transparent; pointer-events: auto; }
.range-input::-webkit-slider-runnable-track { height: 4px; border-radius: 999px; background: #198754; }
.range-input::-webkit-slider-thumb { appearance: none; width: 16px; height: 16px; border-radius: 50%; background: #d1fae5; border: 2px solid #198754; margin-top: -6px; cursor: pointer; }
.range-input::-moz-range-track { height: 4px; border-radius: 999px; background: #198754; }
.range-input::-moz-range-thumb { width: 16px; height: 16px; border-radius: 50%; background: #d1fae5; border: 2px solid #198754; cursor: pointer; }
.customer-modal-dialog { max-width: 1150px; }
.customer-table-wrap { border: 1px solid #dee2e6; border-radius: 12px; overflow: hidden; background: #fff; }
.customer-table { width: 100%; table-layout: fixed; }
.customer-table thead th { font-size: 14px; font-weight: 700; padding: 12px 10px; vertical-align: middle; white-space: nowrap; }
.customer-table tbody td { font-size: 14px; padding: 12px 10px; vertical-align: middle; }
.customer-table tbody tr:hover { background: #f8fafc; }
.customer-address-cell { line-height: 1.45; white-space: normal; word-break: break-word; }
.customer-pick-btn { min-width: 64px; }
.order-tab-btn { display: inline-flex; align-items: center; gap: 8px; padding: 6px 10px; }
.order-tab-label { white-space: nowrap; }
.order-count-badge { min-width: 18px; height: 18px; padding: 0 5px; border-radius: 999px; background: #dc3545; color: #fff; font-size: 11px; font-weight: 700; display: inline-flex; align-items: center; justify-content: center; line-height: 1; flex-shrink: 0; }
.tab-x { opacity: 0.85; cursor: pointer; margin-left: 2px; flex-shrink: 0; }
.tab-x:hover { opacity: 1; }
.custom-toast { min-width: 320px; max-width: 420px; background: #fff; border-radius: 12px; border-left: 5px solid #dc3545; display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; padding: 14px 14px 14px 16px; color: #1f2937; }
.custom-toast-content { flex: 1; min-width: 0; }
.custom-toast-title { font-size: 16px; font-weight: 700; margin-bottom: 4px; color: #111827; }
.custom-toast-message { font-size: 14px; line-height: 1.45; color: #4b5563; word-break: break-word; }
.custom-toast-close { border: 0; background: transparent; color: #9ca3af; font-size: 20px; line-height: 1; cursor: pointer; padding: 0; margin-top: -2px; }
.custom-toast-close:hover { color: #374151; }
...
.shipping-brand-box { border: 1px dashed #d1d5db; border-radius: 12px; padding: 14px; background: #fff; display: flex; justify-content: center; align-items: center; }
.shipping-brand-logo { max-height: 52px; width: auto; object-fit: contain; }
</style>
